    ...
    try:
        ...
        if os.path.exists(f"{art_keywork}.txt"):
            ...
            if read_all != article:
                ...
                try:
                    # 从内容中提取标题
                    extracted_title = "Anthropic新闻"  # 默认标题
                    if article:
                    ...
                    success = push_logger.log_push(
                        ...
                        category="AI资讯",
                        url=None,  # 使用None，避免固定URL导致去重（新闻内容会变化）
                        push_channel="无",  # 不推送
                        push_status="skipped",  # 跳过推送
                        keywords=["Anthropic", "AI资讯"]
                    )
                    if success:
