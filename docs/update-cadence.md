...
| Coding / Agent / Model 横评 | `coding-plan.html`、`agent-plan.html`、`model-plan.html`、`local-model-plan.html`、`low-filter-api-plan.html` | 套餐价、默认模型、兼容矩阵、Agent 用量池；本地页核对排行榜/显存/街货价；低过滤云 API 核对 Venice / NinjaChat 价表 |
